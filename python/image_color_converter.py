...
def outputName(fn: str):
    ...
    return fn if fn.find('/') == -1 else IsADirectoryError(fn)
...
if __name__ == "__main__": 
    ...
    data = np.array(img.convert('RGBA'))   # "data" is a height x width x 4 numpy array

    # Replace white with red... (leaves alpha values alone...)
    visible_areas = data[:,:,3] != 0
    data[:,:,:3][visible_areas] = args.rgb # Transpose back needed
